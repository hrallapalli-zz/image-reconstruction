classdef BrukerDataSuperclass
    ...
    % All Rights Reserved
    %
    % $Id: BrukerDataSuperclass.m,v 1.2 2013/05/24 13:29:27 haas Exp $
    %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    properties (SetAccess=protected, Hidden)
        PackageParameters=struct( 'MatlabPackageName', 'pvmatlab - MATLAB package for handling ParaVision data', 'MatlabPackageVersion', '0.CVS.1'); % Packagename, Version etc.
            ...
            if sum(strcmp(varargin, 'imagewrite'))>=1
                ...
                if sum(strcmpi(varargin, 'auto'))>=1
                    ...
                    if ~isempty(pdata_pos)
                        pos2=strfind(obj.Filespath.auto(1:pdata_pos-1), '/');
                        expno=obj.Filespath.auto(1:pos2(end)-1);
                        clear pos2;
                        obj.Filespath.path_to_acqp=[expno, '/acqp'];
                        obj.Filespath.path_to_method=[expno, '/method'];
                        obj.Filespath.path_to_fid=[expno, '/fid'];
                        obj.Filespath.path_to_visu_template=[expno, '/visu_pars'];
                        pos3=strfind(expno,'/');
                        if ~isempty(pos3)
                            obj.Filespath.path_to_subject=[expno(1:pos3(end)-1), '/subject'];
                        else % only 2 or ''
                            tmp=dir;
                            for i=1:length(tmp)
                        ...
                        if length(obj.Filespath.auto)>=pdata_pos+6 % pos=p of pdata -> pos+6-> is the X in pdata/X
                            pos4=strfind(obj.Filespath.auto,'/');
                            while pos4(end)>pdata_pos+5
                                obj.Filespath.auto=obj.Filespath.auto(1:pos4(end)-1);
                                pos4(end)=[];
                            end                       
                            obj.Filespath.path_to_visu=[obj.Filespath.auto, '/visu_pars'];
                            obj.Filespath.path_to_reco=[obj.Filespath.auto, '/reco'];
                            obj.Filespath.path_to_2dseq=[obj.Filespath.auto, '/2dseq'];
                            obj.Filespath.mode='procno';
                        else
                            obj.Filespath.auto=expno;
                        end
                    else % String doesn't contain 'pdata'
                        pos2=strfind(obj.Filespath.auto, '/');
